...
生成器的中间文本。H3 / Seedance / LTX-2.5 视频，以及 Seedream 和 Qwen-Image 图像打包，是建立在公共契约上的
...
实验中的 VLM scorer 属于事后诊断：它从已经生成的 RAW/PE 视频中抽帧、对 pair 评分并写出
汇总结果。`expand` 不会调用它，评分也不会选择或修订提示词候选。因此当前 PE 生命周期包含
结构校验与有界修复；由 VLM 引导的生成、排序和迭代提示词修订仍是后续优化工作。
