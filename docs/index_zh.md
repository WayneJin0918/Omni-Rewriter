# Omni-Rewriter 文档

[English index](index.md) · [项目 README](../README.md)

Omni-Rewriter 是面向多模态生成的类型化、可校验 **提示词扩展（PE）Agent Harness**。
扩展与媒体生成是分开的步骤。

## 从这里开始

- [快速开始](getting-started_zh.md)
- [架构](architecture_zh.md)
- [评测](evaluation_zh.md)

## 方言与适配器

权威目录：[`dialects/`](dialects/README.md)

- [视频 PE（H3）](dialects/h3-pe-harness_zh.md)
- [Seedance PE](dialects/seedance-pe_zh.md)
- [图像 PE](dialects/image-pe_zh.md)
- [H3 adapters](dialects/h3-adapters_zh.md)
- [生成适配器](dialects/generation-adapters_zh.md)
- [社区模型待办](dialects/community-models_zh.md)
- [公开 H3 参考](references/README.md)
- [请求示例](../examples/requests/)

## 公开演示与宣传

见 [`site/README.md`](site/README.md)。

- [H3 PE 站点](day2-h3-pe/index.html) — 短片进入页 → [`home.html`](day2-h3-pe/home.html)
- [H3 PE showcase](h3-pe-showcase/index.html)
- [精简视频 Gallery](assets/gallery/) · [图像 Gallery](assets/gallery/image/)
- [宣传流水线](promo/README.md) · [中文宣传稿](promo/copy_zh.md) / [EN](promo/copy_en.md)

## 项目治理

- [路线图](ROADMAP.md)
- [贡献指南](CONTRIBUTING.md)
- [安全策略](SECURITY.md)
- [行为准则](CODE_OF_CONDUCT.md)
- [Agent 备注](AGENTS.md)
